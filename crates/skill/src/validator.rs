...
fn validate_skill_structure(skill: &crate::model::Skill) -> Vec<String> {
    ...
    if skill.scripts.is_empty() && skill.references.is_empty() && skill.assets.is_empty() {
    ...
    for script in &skill.scripts {
        if script.contains("..") {
            errors.push(format!("Invalid script path (contains '..'): {}", script));
        }
    }
...
mod tests {
    ...
    fn test_validate_skill_file() {
        let temp_dir = TempDir::new().unwrap();
        let skill_dir = create_test_skill_dir(temp_dir.path(), "packaged-skill");
    ...
    fn test_validate_skill_with_spaces_in_name() {
        ...
        let skill = crate::model::Skill {
            ...
            content: String::new(),
            source: crate::model::SkillSource::SkillMd(std::path::PathBuf::new()),
            scripts: vec![],
            references: vec![],
            assets: vec![],
        };
        ...
        assert!(errors.iter().any(|e| e.contains("spaces")), "Expected error about spaces in name, got: {:?}", errors);
    }
