...
impl ApiError {
    pub fn new(status: Status, error: impl Into<String>, code: &'static str) -> Self {
        Self {
            status,
            body: ErrorBody {
                error: error.into(),
...
impl From<ConfigError> for ApiError {
    fn from(e: ConfigError) -> Self {
        match e {
            ConfigError::ResourceNotFound { resource_type, name } => ApiError::new(
                Status::NotFound,
                format!("{resource_type} '{name}' not found"),
                "RESOURCE_NOT_FOUND",
            ),
            ConfigError::ResourceExists { resource_type, name } => ApiError::new(
                Status::Conflict,
            ...
            ConfigError::Json(e) => {
                ApiError::new(Status::BadRequest, e.to_string(), "JSON_PARSE_ERROR")
            }
            ConfigError::Io(e) => {
                ApiError::new(Status::InternalServerError, e.to_string(), "IO_ERROR")
            }
        }
    }
}
...
pub type ApiCreated<T> = Result<(Status, rocket::serde::json::Json<T>), ApiError>;
