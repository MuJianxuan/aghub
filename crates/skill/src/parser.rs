...
pub fn parse_skill_file(path: &Path) -> Result<Skill> {
    ...
    let content = skill_md_content.ok_or_else(|| SkillError::MissingSkillMd {
...
pub fn parse_skill_dir(path: &Path) -> Result<Skill> {
    ...
    let skill_md = skills_ref::parser::find_skill_md(path)
        .ok_or_else(|| SkillError::MissingSkillMd {
            path: path.to_path_buf(),
        })?;
...
pub fn parse(path: &Path) -> Result<Skill> {
    ...
    if path.is_dir() {
        ...
        parse_skill_file(path)
    } else if path_str.ends_with(".md") || path.file_name() == Some("SKILL.md".as_ref()) {
