...
impl AgentType {
	...
	pub fn as_str(&self) -> &'static str {
		match self {
			...
			AgentType::Warp => "warp",
			AgentType::Trae => "trae",
			AgentType::AmazonQCli => "amazonqcli",
			AgentType::Firebender => "firebender",
...
impl std::str::FromStr for AgentType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match s.to_lowercase().as_str() {
			...
			"firebase" => Ok(AgentType::Firebase),
			"openhands" => Ok(AgentType::OpenHands),
			"gemini-cli" | "geminicli" => Ok(AgentType::GeminiCli),
			"jules" => Ok(AgentType::Jules),
			...
			"amazonqcli" | "amazonq" | "q" => Ok(AgentType::AmazonQCli),
			"firebender" => Ok(AgentType::Firebender),
